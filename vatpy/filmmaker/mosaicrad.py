...
def plot_mosaicrad(start, end):
    ...
    vmin_HUV, vmax_HUV = -16, -12
    vmin_XRAY, vmax_XRAY = -16, -12
    ...
    cmap_bfield = 'plasma'
    cmap_HUV = 'cmr.eclipse'
    cmap_XRAY = 'cmr.voltage'
    ...
    for i in range(start, end+1):
        ...
        # Gaseous component:
        quantity = ['mass', 'temp', 'HI', 'H2', 'HII', 'bfield', 'FUVTOT',
                    'FXRAYTOT']
        ...
        fig, ax = plt.subplots(4, 6, figsize=(16, 3/6 * 16),
        ...
        # Hard UV radiation:
        im = ax[0, 5].imshow(np.log10(im_gas_face['FUVTOT']),
                             extent=im_gas_face['extent'], origin='lower',
                             vmin=vmin_HUV, vmax=vmax_HUV, cmap=cmap_HUV)
        ax[0, 5].set_facecolor(plt.get_cmap(cmap_HUV)(0))
        ax[0, 5].set_aspect('equal')
        ax[0, 5].set_xticks([])
        ax[0, 5].set_yticks([])
        cax = ax[0, 5].inset_axes([0.05, 0.93, 0.6, 0.03])
        cbar = fig.colorbar(im, cax=cax, orientation='horizontal')
        cbar.set_label(color='w', label=r'$\log_{10}(F \ [\text{s}^{-1} \ $' +
                       r'$\text{cm}^{-2}])$')
        cbar.ax.tick_params(colors='w', which='both')
        cbar.outline.set_edgecolor('w')
        ax[0, 5].text(0.95, 0.05, 'UV', c='w', ha='right', va='bottom',
                      transform=ax[0, 5].transAxes, fontsize='medium')

        im = ax[1, 5].imshow(np.log10(im_gas_edge['FUVTOT']),
                             extent=im_gas_edge['extent'], origin='lower',
                             vmin=vmin_HUV, vmax=vmax_HUV, cmap=cmap_HUV)
        ax[1, 5].set_facecolor(plt.get_cmap(cmap_HUV)(0))
        ax[1, 5].set_ylim(box[0]/2, box[1]/2)
        ...
        # X-ray radiation:
        im = ax[2, 5].imshow(np.log10(im_gas_face['FXRAYTOT']),
                             extent=im_gas_face['extent'], origin='lower',
                             vmin=vmin_XRAY, vmax=vmax_XRAY,
                             cmap=cmap_XRAY)
        ax[2, 5].set_facecolor(plt.get_cmap(cmap_XRAY)(0))
        ax[2, 5].set_aspect('equal')
        ax[2, 5].set_xticks([])
        ax[2, 5].set_yticks([])
        cax = ax[2, 5].inset_axes([0.05, 0.93, 0.6, 0.03])
        cbar = fig.colorbar(im, cax=cax, orientation='horizontal')
        cbar.set_label(color='w', label=r'$\log_{10}(F \ [\text{s}^{-1} \ $' +
                       r'$\text{cm}^{-2}])$')
        cbar.ax.tick_params(colors='w', which='both')
        cbar.outline.set_edgecolor('w')
        ax[2, 5].text(0.95, 0.05, 'X-RAY', c='w', ha='right', va='bottom',
                      transform=ax[2, 5].transAxes, fontsize='medium')

        im = ax[3, 5].imshow(np.log10(im_gas_edge['FXRAYTOT']),
                             extent=im_gas_edge['extent'], origin='lower',
                             vmin=vmin_XRAY, vmax=vmax_XRAY,
                             cmap=cmap_XRAY)
        ax[3, 5].set_facecolor(plt.get_cmap(cmap_XRAY)(0))
        ax[3, 5].set_ylim(box[0]/2, box[1]/2)
        ax[3, 5].set_aspect('equal')
        ax[3, 5].set_xticks(xticks[1:-1])
        ax[3, 5].set_yticks([])
        ax[3, 5].set_xlabel('$x$ [kpc]')

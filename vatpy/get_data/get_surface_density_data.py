...
# -------------- Declare function(s)
def get_surface_density_data(output_dir, n, N, r_bins_gas, r_bins_dm,
                             r_bins_disc, r_bins_nsc, r_bins_stars, zcut_halo,
                             zcut_galaxy):
    '''TODO
    '''
    ...
    for i in range(n, N+1):
        ...
        # Limit the data in Z:
        if zcut_halo:
            mass_dm = mass_dm[np.abs(pos_dm[:, 2]) < zcut_halo]
            pos_dm = pos_dm[np.abs(pos_dm[:, 2]) < zcut_halo]

        if zcut_galaxy:
            mass_gas = mass_gas[np.abs(pos_gas[:, 2]) < zcut_galaxy]
            pos_gas = pos_gas[np.abs(pos_gas[:, 2]) < zcut_galaxy]
            if 'PartType2' in h.keys():
                mass_disc = mass_disc[np.abs(pos_disc[:, 2]) < zcut_galaxy]
                pos_disc = pos_disc[np.abs(pos_disc[:, 2]) < zcut_galaxy]
            if 'PartType3' in h.keys():
                mass_nsc = mass_nsc[np.abs(pos_nsc[:, 2]) < zcut_galaxy]
                pos_nsc = pos_nsc[np.abs(pos_nsc[:, 2]) < zcut_galaxy]
            if 'PartType4' in h.keys():
                mass_stars = mass_stars[np.abs(pos_stars[:, 2]) < zcut_galaxy]
                pos_stars = pos_stars[np.abs(pos_stars[:, 2]) < zcut_galaxy]
